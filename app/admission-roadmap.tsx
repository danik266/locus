...
export function AdmissionRoadmap({profile,program,done,onToggle,onBack,onEdit,onClear}:Props){
 ...
 const nextStage=stages.find(stage=>!done.includes(`roadmap-${program.id}-${stage.id}`));
 const intro=insight?.intro??c.fallbackIntro;
 const priority=insight?.priority??c.fallbackPriority;
 if(loading)return <section className="workspace container roadmap-page roadmap-loading" aria-live="polite"><div className="workspace-top"><button className="back-button" onClick={onBack}>{c.back}</button><span className="eyebrow">{c.eyebrow}</span></div><div className="roadmap-loading-body"><div className="roadmap-loading-symbol" aria-hidden="true"><svg viewBox="0 0 240 148"><path className="roadmap-loading-track" d="M24 26 H180 A24 24 0 0 1 180 74 H60 A24 24 0 0 0 60 122 H214"/><path className="roadmap-loading-trace" pathLength="1" d="M24 26 H180 A24 24 0 0 1 180 74 H60 A24 24 0 0 0 60 122 H214"/><circle className="roadmap-loading-goal-halo" cx="214" cy="122" r="24"/><circle className="roadmap-loading-goal" cx="214" cy="122" r="18"/><polyline points="207,129 221,115 212,115 221,115 221,124"/></svg></div><h1>{c.loading}</h1><p>{c.loadingSub}</p><div className="roadmap-loading-school">{program.school} · {program.title}</div><ol>{c.loadingSteps.map((step,i)=><li className={i<phase?'complete':i===phase?'active':''} key={step}><span>{i<phase?'✓':`0${i+1}`}</span>{step}</li>)}</ol></div></section>;
 return <section className="workspace container roadmap-page"><div className="workspace-top"><button className="back-button" onClick={onBack}>{c.back}</button><button className="text-button" onClick={onEdit}>{c.edit}</button></div>
  <header className="roadmap-hero"><div className="roadmap-hero-copy"><span className="roadmap-kicker"><i/>{insight?c.ai:c.fallback}</span><span className="roadmap-eyebrow">{c.eyebrow}</span><h1>{c.title}</h1><p>{c.subtitle}</p><div className="roadmap-target"><span>{c.program}</span><strong>{program.school}</strong><small>{program.title} · {program.country} · {c.year} {profile.year}</small></div></div><div className="roadmap-hero-art" aria-hidden="true">
   <div className="roadmap-art-panel">
    ...
    <div className="roadmap-art-steps">
     <div className="roadmap-art-step"><span className="roadmap-art-marker">01</span><div className="roadmap-art-chip"><strong>{c.artStart}</strong><small>{c.artStartSub}</small></div></div>
     <div className="roadmap-art-step"><span className="roadmap-art-marker">04</span><div className="roadmap-art-chip"><strong>{c.artMiddle}</strong><small>{c.artMiddleSub}</small></div></div>
     <div className="roadmap-art-step"><span className="roadmap-art-marker roadmap-art-marker-end">08</span><div className="roadmap-art-chip"><strong>{c.artEnd}</strong><small>{c.artEndSub}</small></div></div>
    </div>
    <div className="roadmap-art-foot"><span>{c.artFoot}</span><span>01 — 08</span></div>
   </div>
  </div></header>
  <div className="roadmap-summary"><div><span className="eyebrow">{c.priority}</span><h2>{priority}</h2><p>{intro}</p>{nextStage&&<div className="roadmap-immediate"><strong>{locale==='ru'?'Следующее действие':locale==='en'?'Next action':'Келесі әрекет'}</strong><p>{nextStage.actions[0]}</p><a href={nextStage.sourceUrl} target="_blank" rel="noopener noreferrer">{c.source}</a></div>}{nextStage&&<button className="button small roadmap-next" onClick={()=>document.getElementById(`stage-${nextStage.id}`)?.scrollIntoView({behavior:'smooth',block:'center'})}>{locale==='ru'?'К следующему этапу ↗':locale==='en'?'Go to next stage ↗':'Келесі кезеңге өту ↗'}</button>}</div><div className="roadmap-progress"><span>{c.progress}</span><strong>{progress}<small>%</small></strong><div className="roadmap-progress-track"><i style={{width:`${progress}%`}}/></div><small>{complete} / {stages.length} {c.complete}</small></div></div>
  <AdmissionAssistant programId={program.id} nextStage={nextStage?.title??(locale==='ru'?'Все этапы отмечены':locale==='en'?'All stages completed':'Барлық кезеңдер орындалды')}/>
